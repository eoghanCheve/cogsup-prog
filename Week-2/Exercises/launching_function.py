...
def launching_function(tgap = 0, sgap = 0, speeddiff = 0):
    ...
    while condition:
        ...
        if rsquare.absolute_position[0] <= -50 + sgap:
            rsquare.move(offset=(4,0))
            exp.clock.reset_stopwatch()
        elif time >= tgap:
            gsquare.move(offset=(4 + speeddiff,0))
...
launching_function()
launching_function(tgap = 200)
launching_function(sgap = 50)
launching_function(speeddiff = 8)
